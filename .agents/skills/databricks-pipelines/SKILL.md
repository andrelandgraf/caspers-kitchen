...
- **Changing dataset type** → cannot change ST→MV or MV→ST without manually dropping the existing table first. Full refresh does NOT help — the pipeline rejects the type change before it even starts processing. You MUST `DROP TABLE IF EXISTS catalog.schema.table` for every table whose type changed, then deploy, then run with `--full-refresh`. If you changed multiple tables in one commit, drop them ALL before deploying. Failure to do this breaks the pipeline silently (it stays in RUNNING state but processes zero rows), causing downstream jobs to operate on stale data.
