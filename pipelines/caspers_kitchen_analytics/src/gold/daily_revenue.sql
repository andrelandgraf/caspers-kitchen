CREATE OR REFRESH MATERIALIZED VIEW `caspers-kitchen-prod`.gold.daily_revenue
...
WITH order_totals AS (
  ...
  FROM `caspers-kitchen-prod`.silver.orders o
...
items_sold AS (
  ...
  FROM `caspers-kitchen-prod`.silver.order_items oi
  JOIN `caspers-kitchen-prod`.silver.orders o ON oi.order_id = o.id
  GROUP BY DATE(o.created_at)
),
refund_totals AS (
  SELECT
  ...
  JOIN `caspers-kitchen-prod`.silver.orders o ON r.order_id = o.id
...
top_categories AS (
  ...
  FROM (
    ...
    FROM `caspers-kitchen-prod`.silver.order_items oi
    JOIN `caspers-kitchen-prod`.silver.orders o ON oi.order_id = o.id
    JOIN `caspers-kitchen-prod`.silver.menu_items mi ON oi.menu_item_id = mi.id
