CREATE OR REFRESH MATERIALIZED VIEW `caspers-kitchen-prod`.gold.support_case_context
...
case_refunds AS (
  ...
  FROM `caspers-kitchen-prod`.silver.refunds
...
case_credits AS (
  ...
  FROM `caspers-kitchen-prod`.silver.credits
  WHERE support_case_id IS NOT NULL
  GROUP BY support_case_id
),
user_lifetime AS (
  ...
  FROM `caspers-kitchen-prod`.silver.orders
  GROUP BY user_id
),
user_recent_cases AS (
  ...
  FROM `caspers-kitchen-prod`.silver.support_cases
...
JOIN `caspers-kitchen-prod`.silver.users u ON sc.user_id = u.id
